...
├── image_generator.py      # 图表生成器（依赖 matplotlib + numpy，可选）
...
> **`images/` 说明**：包含 32 张预生成的类型卡片图片，文件名即类型码（如 `MCRD.png`、`PGEC.png`）。BOND 16 型 + ECHO 16 型各一张，扁平存放于目录下，无子目录。
...
| 文件 | 说明 |
|------|------|
| `profiler.py` | 主 CLI 入口，负责参数解析、流水线调度、输出路由 |
| `data_parser.py` | 数据解析器，支持 5 种 JSON 格式 + 目录结构自动识别 |
| `feature_extractor.py` | 统一特征提取器，从 bundle 中提取 BOND 和 ECHO 的行为特征 |
| `bond_classifier.py` | BOND Profile 分类器，行为特征 → 四维连续得分 → 16 型 |
| `echo_classifier.py` | ECHO Matrix 分类器，Agent 特征 → 四维连续得分 → 16 型 |
| `sync_matcher.py` | PARTS Spectrum 匹配器，BOND×ECHO → 五维评分 → 关系 10 型匹配 |
| `card_generator.py` | Markdown 报告生成器，将三层结果渲染为可视化卡片 |
| `image_generator.py` | 图表生成器，生成 BOND/ECHO 柱状图 + PARTS 雷达图（依赖 matplotlib + numpy，可选） |
| `all_lexicons.py` | 词库层，10 个词法分析器，中英文混合分词（1/2/3-gram） |
| `type_definitions.py` | 类型定义，BOND 16 型 / ECHO 16 型 / PARTS 10 型的元数据与理想向量 |
| `utils.py` | 通用工具函数（sigmoid、中英文分词、多样性指标计算） |
| `mock_scenarios.py` | 内置 3 个 Mock 演示场景，用于 `--demo` 模式 |
| `__init__.py` | 包入口，导出 `run_profile`、`FeatureExtractor` 等核心接口 |
| `images/` | 预生成的 32 张 BOND/ECHO 类型卡片图片（PNG），文件名为类型码（如 `MCRD.png`） |
